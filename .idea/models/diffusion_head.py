...
class DiffusionHead(nn.Module):
    ...
    def model_predictions(self, backbone_feats, images_whwh, x, t, lost_features=None, fix_bboxes=False,
        ...
        outputs_class, outputs_coord, outputs_score = self.head(backbone_feats, torch.split(bboxes, bs, dim=0), t,
                                                                lost_features, fix_bboxes)
        end_time = time.time()
    ...
    def new_ddim_sample(self, backbone_feats, images_whwh, ref_targets=None, dynamic_time=True, num_timesteps=1,
                        num_proposals=500, inference_time_range=1, track_candidate=1, diffusion_t=200,
                        clip_denoised=True):
        ...
        track_t_list = []
        total_time = 0
        asso_matrix = torch.randn(self.num_proposals, self.num_proposals, device=self.device, dtype=self.dtype)
        if ref_targets is None or self.track_candidate == 0:
            ref_bboxes = torch.randn(shape, device=self.device)
            for i in range(batch):
                t = torch.randint(self.num_timesteps - self.inference_time_range, self.num_timesteps, (2,),
            ...
            for batch_idx, num_gt in enumerate(nlabel):
                ...
                d_boxes, score, d_noise, ref_label = self.prepare_diffusion_concat(gt_boxes, ref_t, scores=asso_matrix)
                diffused_boxes.append(d_boxes)
                ref_t_list.append(ref_t)
                d_boxes, score, d_noise, ref_label = self.prepare_diffusion_concat(gt_boxes, track_t, ref_label,
                                                                                   scores=asso_matrix)
        ...
        for sampling_timestep in range(sampling_timesteps):
            ...
            preds, outputs_class, outputs_coord, outputs_score, association_time = self.model_predictions(
                backbone_feats, images_whwh, bboxes, time_cond, fix_bboxes=False,
            ...
            def diffusion(sampling_times, bboxes, x_start, pred_noise):
                ...
                alpha = torch.tensor([self.alphas_cumprod[time] for time in times], dtype=self.dtype,
                                     device=self.device)
                alpha_next = torch.tensor([self.alphas_cumprod[time_next] for time_next in time_nexts],
                ...
                if self.box_renewal:
                    for i in range(batch):
                        ...
                        bboxes[i] = torch.cat(
                            (bboxes[i], torch.randn(self.num_proposals - remain_list[i], 4, device=self.device)), dim=0)
                else:
            ...
            bboxes[:batch] = diffusion(ref_t_time_pairs_list[:, sampling_timestep], bboxes[:batch], x_start[:batch],
                                       pred_noise[:batch])
            bboxes[batch:] = diffusion(track_t_time_pairs_list[:, sampling_timestep], bboxes[batch:], x_start[batch:],
                                       pred_noise[batch:])
    ...
    def forward(self, features, mate_info, targets=None):
        ...
        if self.training:
            targets, x_boxes, noises, t, ass_score = self.prepare_targets(targets, images_whwh)
            print(ass_score)
            t = t.squeeze(-1)
            # t[b:]=t[:b]
            x_boxes = x_boxes * images_whwh[:, None, :]
    ...
    def prepare_diffusion_concat(self, gt_boxes, t, ref_mask=None, scores=None):
        """
        :param gt_boxes: (cx, cy, w, h), normalized
        :param num_proposals:
        """
        ...
        noise_score = torch.randn(self.num_proposals, self.num_proposals, device=self.device, dtype=self.dtype)
        # noise.shape [500,4]
        select_mask = None
        ...
        # noise sample
        x = self.q_sample(x_start=x_start, t=t, noise=noise)
        x_as_score = self.q_sample(x_start=scores, t=t, noise=noise_score)
        ...
        if self.training:
            ...
            x_as_score = torch.clamp(x_as_score, min=-1 * self.scale, max=self.scale)
            x_as_score = ((x_as_score / self.scale) + 1) / 2.
        ...
        return diff_boxes, score, noise, select_mask
    ...
    def prepare_targets(self, targets, images_whwh):
        ...
        for batch_idx, num_gt in enumerate(nlabel):
            ...
            gt_track_id = labels[batch_idx, :num_gt, 5]
            ...
            d_boxes, score, d_noise, d_mask = self.prepare_diffusion_concat(x_gt_boxes, d_t,
                                                                             select_mask.get(
                                                                                 batch_idx % (len(nlabel) // 2),
                                                                                 None), associated_scores)
        ...
        return new_targets, torch.stack(diffused_boxes), torch.stack(noises), torch.stack(ts), torch.stack(ass_score)

    def get_scores(self, targets):
        labels = targets[..., :6]
        nlabel = (labels.sum(dim=2) > 0).sum(dim=1)
        gt_ids = []
        ts = []
        num_targets = max(500, 500)
        association_matrix = np.zeros((num_targets, num_targets), dtype=np.float64)
        association_matrix = torch.from_numpy(association_matrix)

        for batch_idx, num_gt in enumerate(nlabel):
            gt_track_id = labels[batch_idx, :num_gt, 5]
            gt_ids.append(gt_track_id)

        for i, track_id1 in enumerate(gt_ids[0]):
            for j, track_id2 in enumerate(gt_ids[1]):
