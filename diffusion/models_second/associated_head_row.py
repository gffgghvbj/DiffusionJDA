...
class D2MP_OB(Module):
    ...
    def loss_associated_focal(self, outputs, gt):
        ...
        # outputs = outputs[:, :num_boxes]
        diff_matrix = (outputs != gt)
        num_mismatch = diff_matrix.sum().item()
        acc = []
        test_p = []
        test_r = []
        output = outputs
        target = gt.reshape(-1, num_gt, num_bboxes)
        output = output.reshape(-1, num_gt, num_bboxes)
        ...
        loss = hungarian_loss(output, target)
        ic(loss)
        ...
        #
        # print('P {:.2f}% \t R {:.2f}% \t weighted Accuracy {:.2f} %'.format(100.0 * np.mean(np.array(test_p)), 100.0 * np.mean(np.array(test_r)),
...
class AssociatedHead(Module):
    ...
    def asso_generate(self, targets=None, imgs=None):
        ...
        pre_gt_boxes = torch.tensor(pre_gt_boxes).detach()
        cur_gt_boxes = torch.tensor(cur_gt_boxes).detach()
        ...
        ass_matrix = ass_matrix[:, :gt_cur].cpu().numpy()
        return ass_matrix
    ...
    def forward(self, mate_info, targets=None, imgs=None):
        ...
        if self.training:
            ...
            if gt_pre != 0:
                reid_feature_pre = self.reid_head.compute_embedding(pre_images, pre_gt_boxes[:, :4], tag="pre")
                reid_feature_curr = self.reid_head.compute_embedding(cur_images, cur_gt_boxes[:, :4], tag="curr")
                pos_prev = self._get_pos_encoding(pre_gt_boxes[:, :4])  # [N, D]
                pos_curr = self._get_pos_encoding(cur_gt_boxes[:, :4])

                attn_prev, _ = self.cross_attn_prev(
                    query=(reid_feature_pre + pos_prev).unsqueeze(0).float(),
                    key=(reid_feature_curr + pos_curr).unsqueeze(0),
                    value=reid_feature_curr.unsqueeze(0).float()
                )
                attn_curr, _ = self.cross_attn_curr(
                    query=(reid_feature_curr + pos_curr).unsqueeze(0).float(),
                    key=(reid_feature_pre + pos_prev).unsqueeze(0).float(),
