...
class DiffusionTracker(object):
    ...
    def update(self, cur_image):
        ...
        if self.frame_id == 1:
            ...
            dets_embs = self.asso_model.get_reid_emb(cur_image, detections[:, :4], "cur").cpu()
            self.pre_imgs = cur_image
            for det, emb in zip(detections, dets_embs):
            ...
            imgs = (self.pre_imgs, self.pre_imgs)
            ref_bboxes = [STrack.tlwh_to_tlbr(track._tlwh) for track in self.tracked_stracks]
            trk_embs = [track.get_emb() for track in self.tracked_stracks]
            ...
            scores = diffusion_ref_detections[:, 5]
            remain_inds = scores > 0.6
            inds_low = scores > 0.1
            inds_high = scores < 0.6

            inds_second = np.logical_and(inds_low, inds_high)
            ref_dets_second = diffusion_ref_detections[inds_second]
            ref_dets = diffusion_ref_detections[remain_inds]
            track_dets_second = diffusion_track_detections[inds_second]
            track_dets = diffusion_track_detections[remain_inds]
            ...
            pre_dets_embs = self.asso_model.get_reid_emb(self.pre_imgs, ref_dets[:, :4], "pre").cpu()
            cur_dets_embs = self.asso_model.get_reid_emb(cur_image, track_dets[:, :4], "cur").cpu()
            detections_embs = self.asso_model.get_reid_emb(cur_image, detections[:, :4], "cur").cpu()

            # trust = (track_dets[:, 5] - 0.6) / (1 - 0.6)
            # af = 0.95
            # dets_alpha = af + (1 - af) * (1 - trust)
            #
            # trust = (detections[:, 5] - 0.6) / (1 - 0.6)
            # af = 0.95
            # dets_alpha_det = af + (1 - af) * (1 - trust)
            
            
            if len(ref_bboxes) != 0 and ref_dets.size != 0:
                dists = matching.iou_distance(ref_bboxes, ref_dets[:, :4])

                targets = (ref_bboxes, ref_dets[:, :4])
                asso_matrix_dist = self.asso_model.asso_generate(targets, imgs, trk_embs, pre_dets_embs)
                asso_matrix = - asso_matrix_dist
                # ic(asso_matrix)
                # use Deep-OC-Sort
                iou_matrix = 1. - dists
                a = (iou_matrix > 0.3).astype(np.int32)
                if a.sum(1).max() == 1 and a.sum(0).max() == 1:
                    matches, u_track, u_detection = matching.linear_assignment(dists, self.same_thresh)
                else:
                    # asso_matrix = split_cosine_dist(trk_embs, pre_dets_embs)
                    if self.aw_off:
                        w_matrix = compute_aw_new_metric(asso_matrix, w_association_emb=0.5, max_diff=0.5)
                        asso_matrix *= w_matrix
                    else:
                        asso_matrix *= 0.5
                    final_cost = 1 - (iou_matrix + asso_matrix)
                    matches, u_track, u_detection = min_cost_matching(adap_flag=True, max_distance=0.45,
                                                                      tracks=ref_bboxes,
                                                                      detections=ref_dets, cost_matrix=final_cost,
                                                                      )
            else:
                dists = matching.iou_distance(ref_bboxes, ref_dets[:, :4])
            ...
            if len(matches) > 0:
                ...
                dists_fix = matching.iou_distance(track_dets[matches[:, 1], :4], detections[:, :4])
                matches_fix, u_track_fix, u_detection_fix = matching.linear_assignment(dists_fix,
                                                                                       thresh=self.same_thresh)
                if len(matches_fix) > 0:
                    track_dets[matches[:, 1]][matches_fix[:, 0], :4] = detections[matches_fix[:, 1], :4]
                    cur_dets_embs[matches[:, 1]][matches_fix[:, 0]] = detections_embs[matches_fix[:, 1]]
                    # dets_alpha[matches[:, 1]][matches_fix[:, 0], :4] = dets_alpha_det[matches_fix[:, 1], :4]

                # filt detection with tracked result
                detections = detections[u_detection_fix]
                detections_embs = detections_embs[u_detection_fix]
            ...
            r_tracked_stracks = [self.tracked_stracks[i] for i in u_track if
                                 self.tracked_stracks[i].state == TrackState.Tracked]
            ref_bb = [STrack.tlwh_to_tlbr(track._tlwh) for track in r_tracked_stracks]
            dists = matching.iou_distance(ref_bb, ref_dets_second[:, :4])
            # matches, u_track, u_detection_second = matching.linear_assignment(dists, thresh=0.5)
            matches, u_track, u_detection_second = min_cost_matching(adap_flag=True, max_distance=0.45, tracks=ref_bb,
                                                                     detections=ref_dets_second, cost_matrix=dists)
            if len(matches) > 0:
                # fix position with detection result
                dists_fix = matching.iou_distance(track_dets_second[:, :4][matches[:, 1], :4], detections[:, :4])
                matches_fix, u_track_fix, u_detection_fix = matching.linear_assignment(dists_fix,
                                                                                       thresh=self.same_thresh)
                if len(matches_fix) > 0:
                    track_dets_second[matches[:, 1]][matches_fix[:, 0], :4] = detections[matches_fix[:, 1], :4]
                # filt detection with tracked result
                detections = detections[u_detection_fix]
                detections_embs = detections_embs[u_detection_fix]
            for itracked, idet in matches:
                track = r_tracked_stracks[itracked]
                det = track_dets_second[idet]
                new_strack = STrack(STrack.tlbr_to_tlwh(det[:4]), det[5])
                if track.state == TrackState.Tracked:
                    track.update(new_strack, self.frame_id)
                    activated_starcks.append(track)
            ...
            # matches_lost, u_track_lost, u_detection_lost = matching.linear_assignment(dists_lost,
            #                                                                           0.7)
            matches_lost, u_track_lost, u_detection_lost = min_cost_matching(adap_flag=True, max_distance=0.45,
                                                                             tracks=[track.tlbr for track in
                                                                                     self.lost_stracks],
                                                                             detections=detections,
                                                                             cost_matrix=dists_lost)
            for itracked, idet in matches_lost:
                track = self.lost_stracks[itracked]
                det = detections[idet]
                emb = detections_embs[idet]
                new_strack = STrack(STrack.tlbr_to_tlwh(det[:4]), det[5])
                if track.state == TrackState.Tracked:
                    track.update(new_strack, self.frame_id)
                    track.update_emb(emb)
