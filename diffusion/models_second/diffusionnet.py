...
from .associated_head import AssociatedHead
...
class DiffusionNet(nn.Module):
    ...
    def __init__(self, backbone=None, head=None, assohead=None, act="silu"):
        super().__init__()
        self.backbone = backbone
        self.head = head
        self.assohead = assohead
    ...
    def forward(self, x, targets=(None, None), random_flip=False, input_size=None):
        ...
        if self.training:
            ...
            loss_dict = {'asso_loss': self.assohead(mate_info, targets=torch.cat([pre_targets, cur_targets], dim=0),
                                                    imgs=imgs)}
