...
class D2MP_OB(Module):
    ...
    def loss_associated_focal(self, outputs, gt):
        ...
        print('P {:.2f}% \t R {:.2f}% \t F1 {:.2f}% \t weighted Accuracy {:.2f} %'.format(
            100.0 * np.mean(np.array(test_p)),
            100.0 * np.mean(np.array(test_r)),
            100.0 * np.mean(np.array(test_F1)),
    ...
    def forward(self, x_0, context, indices, t=None):
        ...
        if not self.weight:
            ...
            loss = (1 * loss_C + 1 * loss_encoder).sum()
...
class AssociatedHead(Module):
    ...
    def asso_generate(self, targets=None, imgs=None, reid_feature_pre=None, reid_feature_curr=None):
        pre_boxes, cur_boxes = [torch.as_tensor(t).to(self.device) for t in targets]
        pre_images, cur_images = imgs

        if reid_feature_pre is None or reid_feature_curr is None:
            reid_feature_pre = self.get_reid_emb(pre_images, pre_boxes[:, :4], tag="pre")
            reid_feature_curr = self.get_reid_emb(cur_images, cur_boxes[:, :4], tag="curr")
        else:
            reid_feature_pre = torch.as_tensor(reid_feature_pre).to(self.device)
            reid_feature_curr = torch.as_tensor(reid_feature_curr).to(self.device)

        attn_prev = self.cross_attn_prev(
            query=reid_feature_pre.unsqueeze(0).float(),
            key=reid_feature_pre.unsqueeze(0).float(),
            value=reid_feature_pre.unsqueeze(0).float()
        )[0]
        attn_curr = self.cross_attn_curr(
            query=reid_feature_curr.unsqueeze(0).float(),
        ...
        reid_feature_curr_pad = F.pad(reid_feature_curr, (0, 0, 0, self.num_proposals - reid_feature_curr.size(0)))
        asso_score = self.asso_embedding(reid_feature_pre.unsqueeze(0), reid_feature_curr_pad.unsqueeze(0))
        ass_matrix = self.diffusion.sample(asso_score, sample=1, bestof=False, point_dim=256).squeeze(0)
        return ass_matrix[:pre_boxes.size(0), :cur_boxes.size(0)].cpu().numpy()

    def get_reid_emb(self, imgs, bboxes, tag):
        return self.reid_head.compute_embedding(imgs, bboxes, tag=tag)
    ...
    def clear_memory_bank(self):
        """清空ReID特征存储池，释放内存"""
        self.reid_memory_bank.clear()
        import gc
        gc.collect()  # 触发python垃圾回收
        torch.cuda.empty_cache()  # 释放PyTorch显存缓存
    ...
    def forward(self, mate_info, targets=None, imgs=None):
        ...
        if self.training:
            ...
            pre_images, cur_images = imgs
            cur_ids = targets[1]['id']  # 当前帧 track_id: shape [N]
            cur_boxes = targets[1]['boxes_xyxy']  # 当前帧 boxes: [N, 4]

            if cur_ids.numel() == 0:
                return 0

            # 当前帧提取 ReID 特征
            reid_feature_curr = self.get_reid_emb(cur_images, cur_boxes, tag="curr")  # [N, D]

            # 构建 "前一帧" 特征（基于 memory bank）
            fused_features = []
            for i, tid in enumerate(cur_ids):
                curr_feat = reid_feature_curr[i]
                if tid.item() in self.reid_memory_bank:
                    prev_feat = self.reid_memory_bank[tid.item()]
                    fused_feat = 0.5 * prev_feat + 0.5 * curr_feat  # 可调 alpha
                else:
                    fused_feat = curr_feat
                fused_features.append(fused_feat)
                self.reid_memory_bank[tid.item()] = fused_feat.detach()  # 更新 memory

            reid_feature_pre = torch.stack(fused_features)  # 模拟“上一帧”特征
            # 注意力增强
            attn_prev = self.cross_attn_prev(
                query=reid_feature_pre.unsqueeze(0).float(),
                key=reid_feature_pre.unsqueeze(0).float(),
            ...
            attn_prev = attn_prev[0]
            attn_curr = attn_curr[0]

            # 处理数量不匹配（pad + shuffle）
            pad_rows = self.num_proposals - attn_curr.size(0)
            reid_feature_curr_pad = F.pad(attn_curr, (0, 0, 0, pad_rows)).contiguous()
            ...
            asso_gt = self.get_scores(shuffled_indices, attn_prev.size(0)).to(self.device)

            # 计算关联嵌入与 Diffusion Loss
            ass_score_encoder = self.asso_embedding(attn_prev.unsqueeze(0), reid_feature_curr_pad.unsqueeze(0))
            loss = self.diffusion(asso_gt.to(self.dtype), ass_score_encoder, shuffled_indices)
            return loss

        else:
            self.reid_memory_bank = {}  # 推理前清空 memory（可选）
            return 0
