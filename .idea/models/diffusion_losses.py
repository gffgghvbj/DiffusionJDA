...
class SetCriterionDynamicK(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses, use_focal, use_fed_loss):
        ...
        if self.use_fed_loss:
            ...
            cls_weight_fun = lambda: get_fed_loss_cls_weights(dataset_names=cfg.DATASETS.TRAIN,
                                                              freq_weight_power=cfg.MODEL.ROI_BOX_HEAD.FED_LOSS_FREQ_WEIGHT_POWER)  # noqa
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        if len(pred_box_list) != 0:
            ...
            target_boxes_abs_xyxy_re = torch.cat(
                tgt_box_xyxy_list[-len(tgt_box_xyxy_list) // 2:] + tgt_box_xyxy_list[:len(tgt_box_xyxy_list) // 2])
            ...
            loss_giou = 1 - torch.diag(
                box_ops.generalized_box_iou(src_boxes, src_boxes_re, target_boxes_abs_xyxy, target_boxes_abs_xyxy_re))
...
class HungarianMatcherDynamicK(nn.Module):
    ...
    def forward(self, outputs, targets):
        ...
        with torch.no_grad():
            ...
            # conf_score.shape [1, 500, 1]

            # We flatten to compute the cost matrices in a batch
            pred_logits_pre, pred_logits_curr = torch.split(outputs["pred_logits"], bs // 2, dim=0)
            # pred_logits_pre.shape [1, 500, num_classes]
            # pred_logits_curr.shape [1, 500, num_classes]
            out_bbox_pre, out_bbox_curr = torch.split(outputs["pred_boxes"], bs // 2, dim=0)
            # out_bbox_pre.shape [1, 500, 4]
            # out_bbox_curr.shape [1, 500, 4]

            # conf_score.shape [1, 500]
            # out_prob_pre.shape [1, 500, num_classes]
            # out_prob_curr.shape [1, 500, num_classes]
            # conf_score.shape [1, 500]
            if self.use_focal or self.use_fed_loss:
                out_prob_pre = torch.sqrt(
                    pred_logits_pre.sigmoid() * conf_score)  # [batch_size, num_queries, num_classes]
                out_prob_curr = torch.sqrt(pred_logits_curr.sigmoid() * conf_score)
            else:
                out_prob_pre = torch.sqrt(
                    pred_logits_pre.softmax(-1) * conf_score)  # [batch_size, num_queries, num_classes]
            ...
            for batch_idx in range(bs // 2):
                ...
                # bz_boxes_pre.shape [500, 4]
                # bz_gtboxs_abs_xyxy_pre.shape [36, 4]
                # pair_wise_ious.shape [500, 36]
                ...
                cost_giou = -generalized_box_iou(bz_boxes_pre, bz_boxes_curr, bz_gtboxs_abs_xyxy_pre,
                                                 bz_gtboxs_abs_xyxy_curr)

                # Final cost matrix
                cost = self.cost_bbox * (
                            cost_bbox_pre + cost_bbox_curr) / 2 + self.cost_class * cost_class / 2 + self.cost_giou * cost_giou + 100.0 * (
